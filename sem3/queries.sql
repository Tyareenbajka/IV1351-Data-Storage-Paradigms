...
IF EXISTS couting_lessons_view;
	DROP VIEW

IF EXISTS instructor_lessons_current_month_view;
	DROP VIEW

IF EXISTS student_siblings_view;
	DROP VIEW

IF EXISTS ensembles_next_week_schedule_view;
	-- counting lessons
	CREATE VIEW couting_lessons_view
	AS
	SELECT DATE_FORMAT(schedule_date, '%Y-%m') AS 'year_month'
	...
	FROM schedule
	JOIN lesson ON lesson.id = schedule.lesson_id
	...
	SELECT person.name AS 'instructor'
		,COUNT(instructor_id) AS 'Lessons'
		,schedule_date
	FROM schedule
	...
	SELECT DISTINCT COUNT(sibling.student_id) AS 'siblings'
		,CASE 
			WHEN COUNT(sibling.student_id) = 0
				THEN COUNT(person.id)
			WHEN COUNT(person.id) = COUNT(sibling.student_id)
				THEN COUNT(person.id) + 1
			END AS 'students'
	FROM sibling
	...
	SELECT schedule_date
		...
		,time_end
		,genre
		,ensemble_lesson.max_places AS 'max seats'
		,CASE 
			WHEN COUNT(lesson_student.lesson_id) = ensemble_lesson.max_places
				THEN 'Full booked'
			WHEN COUNT(lesson_student.lesson_id) = (ensemble_lesson.max_places - 1)
				THEN 'One seat left'
			WHEN COUNT(lesson_student.lesson_id) = (ensemble_lesson.max_places - 2)
				THEN 'Two seats left'
			WHEN COUNT(lesson_student.lesson_id + 3) < (ensemble_lesson.max_places)
				THEN 'Many seats left'
			END AS 'seats  left'
	FROM schedule
	JOIN lesson ON lesson_id = lesson.id
	...
	-- Queries
	SELECT *
	FROM couting_lessons_view;

	SELECT *
	FROM instructor_lessons_current_month_view;

	SELECT *
	FROM student_siblings_view;

	SELECT *
	FROM ensembles_next_week_schedule_view;
